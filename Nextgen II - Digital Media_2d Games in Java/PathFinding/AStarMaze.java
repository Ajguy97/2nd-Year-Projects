...
public class AStarMaze extends JFrame implements Runnable, MouseListener, MouseMotionListener, KeyListener {
	...
	public void run() {
		...
		while ( 1==1 ) {
			...
			if (isGameRunning) {
				loops++;
				if(firstTime){
					System.out.println("calculating path for the first time");
	...
	public void paint(Graphics g) {
 	   ...
 	   g.setColor(Color.red);
  		for(int i = 0; i< badguy.closedList1.size();i++){
  			int nextXX = badguy.closedList1.get(i).x;
  			int nextYY = badguy.closedList1.get(i).y;
  			g.fillRect(nextXX*20, nextYY*20, 20, 20);
  		}
  		
  	  g.setColor(Color.green);
		for(int i = 0; i< badguy.openList1.size();i++){
			int nextXXX = badguy.openList1.get(i).x;
			int nextYYY = badguy.openList1.get(i).y;
			g.fillRect(nextXXX*20, nextYYY*20, 20, 20);
		}
        ...
        if (badguy.hasPath) {
        	...
        	while(!badguy.path.isEmpty()){
			aNode next = (aNode)badguy.path.pop();
			int nextX = next.x;
			int nextY = next.y;
			g.setColor(Color.blue);	
			g.fillRect(nextX*20, nextY*20, 20, 20);
        	}
